...
public class ManipulateS3sRecord {
    //每新注册一个普通用户都需要新建立一个S3存储桶
...
//    static final String DB_URL = "jdbc:mysql://localhost:3306/RUNOOB";
    ...
    public void search(String sql, boolean show_s3name, boolean show_s3lastmodifiedtime, boolean show_s3ownerid) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (show_s3lastmodifiedtime) {
                    String s3_lastmodifiedtime = rs.getString("s3_lastmodifiedtime");
                    System.out.print("s3存储桶最近修改时间: " + s3_lastmodifiedtime);
                }
                if (show_s3ownerid) {
                    String s3_ownerid = rs.getString("s3_ownerid");
                    System.out.print("s3存储桶普通用户id: " + s3_ownerid);
                }
