...
public class ManipulateDownloadsRecord {
    ...
    public  void search(String sql, boolean show_downloaduserid, boolean show_s3name, boolean show_downloadtime) {
        ...
        try {
            ...
            while (rs.next()) {
                // 通过字段检索 输出数据
                if (show_downloaduserid) {
                    String upload_userid = rs.getString("upload_userid");
                    System.out.print("下载用户id: " + upload_userid);
                }
                if (show_downloadtime) {
                    String upload_time = rs.getString("upload_time");
                    System.out.print("下载用户时间: " + upload_time);
                }
                if (show_s3name) {
